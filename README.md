...
スパムに対する正しいテストケースではありませんが、少なくともプラグインが動作し Turnstile API と連絡していることは確かめられます。  
実際のスパム攻撃については、クラウドフレアの Turnstile ダッシュボードに統計が表示されます。
...
→ 機械的なフォーム操作や不審な送信元IPアドレスなどはここではじかれる  
